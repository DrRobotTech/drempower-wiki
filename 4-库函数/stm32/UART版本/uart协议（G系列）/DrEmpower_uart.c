...
void send_command(uint8_t id_num, char cmd, unsigned char *data,uint8_t rt )
{
    short id_list = (id_num << 5) + cmd;
		command[0] = 0x08;
    command[1] = id_list >> 8;
    command[2] = id_list & 0xFF;
    for (int i = 0; i < 8; i++) 
    {
        command[i + 3] = data[i];
    }
    HAL_UART_Transmit(&SERVO_USART, (uint8_t *)&command, 11, 1000);
    rxIndex=0;
    HAL_Delay(1);    //如果通过CAN接口控制电机(利用USB转CAN模块TTL转CAN模式)，这1ms延时可以去掉，提高通信速率
}
...
{
    int16_t tmp_int16;
    *(((uint8_t*)(&tmp_int16)) + 0) = data[0];//将 data[0] 的值赋给指针所指向的内存位置上存储的值
...
void value2byte()
{
    ...
    while(1)
    {
        if (data_list.type_data[value_index]==0)
        {
            ...
            byte_index += 4;  //用于下一个值的存储位置
            value_index += 1; //指向下一个值的索引
        }
        switch(data_list.type_data[value_index])
        {
        case 0:
...
void format_data( float *value_data, int *type_data,int length, char * str)
{
    ...
    if (strcmp(str,"decode")==0)
    {
         for (int i = 0; i < 8; i++)
        {
            data_list.byte_data[i]=rx_buffer[3+i];
        }
        byte2value();
    }
}
