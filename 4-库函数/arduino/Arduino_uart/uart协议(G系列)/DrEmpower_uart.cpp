...
void receive_data(void)
{
  ...
  if ( rx_buffer[0]==0x08 && sizeof(rx_buffer) / sizeof(rx_buffer[0]) == 11)
  {
    READ_FLAG=1;
...
void format_data( float *value_data, int *type_data,int length, const char * str)
{
    ...
    if (strcmp(str,"decode")==0)
    {
        for (int i = 0; i < 8; i++)
        {
            data_list.byte_data[i]=rx_buffer[i+3];
        }
        byte2value();
    }
}
