...
#define SERVO_RECEIVE_TIMEOUT 10        /* 串口接收超时(ms) */
 ...
 * @brief 使用串口发送命令
 * 
 * @param length 要发送的命令长度
 */
void send_command(size_t length)
{
...
uint8_t read_data(uint8_t mode)
{
  ...
  for (size_t i = 0; i < sizeof(rx_buffer) / sizeof(rx_buffer[0]); i++)
    ...
    for (;;)
    {
      ...
      if (millis() - timeStart > SERVO_RECEIVE_TIMEOUT){
        if (mode == 1) return rx_buffer[i < 1 ? 0 : 1];
        return 0;
      }
    }
    rx_buffer[i] = Serial.read();
    if (rx_buffer[i] == '\n' && mode == 0) return 1;
    else if (rx_buffer[i] == '\0' && mode == 2) return 1;
  }
  return 0;
}

/**
 * @brief 向指定字符串结尾添加一串字符串
 * 
 * @param pstr 
 * @param s 
 * @param type_check 
 * @param type 
 */
void servo_sappend(char *pstr, const char *s, bool type_check = false, char type = 0)
{
  if (type_check && type != 's') return;
  strncat(pstr, s, UINT32_MAX);
}

/**
 * @brief 向指定字符串结尾添加一个浮点数
 * 
 * @param pstr 
 * @param f 
 * @param type_check 
 * @param type 
 */
void servo_sappend(char *pstr, double f, bool type_check = false, char type = 0)
{
  if (type_check && type != 'f') return;
  char s[16];
  uint_fast8_t width = 1;
  for (int tempf = (int)f; tempf != 0 && (tempf /= 10) != 0; width++);
  dtostrf(f, width, DTOSTRF_PREC, s);
  strncat(pstr, s, sizeof(s) / sizeof(s[0]));
}

/**
 * @brief 向指定字符串结尾添加一个整数
 * 
 * @param pstr 
 * @param d 
 * @param type_check 
 * @param type 
 */
void servo_sappend(char *pstr, int d, bool type_check = false, char type = 0)
{
  if (type_check && type != 'd') return;
  char s[16];
  itoa(d, s, 10);
  strncat(pstr, s, sizeof(s) / sizeof(s[0]));
}
 ...
 * 
 * @param pstr 指定的字符串
 * @param format 要添加的字符串
 * @return char* 指定的字符串
 */
char *servo_sprintf(char *pstr, char *format)
{
  strncat(pstr, format, UINT32_MAX);
  return pstr;
}

/**
 * @brief 格式化输出到指定的字符数组
 * 用以代替 sprintf 函数，格式转换字符仅支持 %%、%d、%s、%f，且不支持选择性参数。与 sprintf 的返回值类型也不一样。
 * 新增自动格式转换字符 %z，可以自动判断输入的参数类型，准确输出对应的字符串。
 * 
 * @tparam T 要转化为字符串的参数的类型
 * @tparam Ts 要转化为字符串的参数的类型
 * @param pstr 要输出到的字符数组
 * @param format 格式
 * @param arg 要转化为字符串的参数
 * @param args 要转化为字符串的参数
 * @return char* pstr
 */
template <typename T, typename... Ts>
char *servo_sprintf(char *pstr, char *format, T arg, Ts... args)
{
  do
  {
    if (*format == '%' && *(format + 1) == '%'){
      ...
      servo_sappend(pstr, arg, true, 'f');
      servo_sprintf(pstr, format + 2, args...);
      return pstr;
    }
    else if (*format == '%' && *(format + 1) == 'd'){
      servo_sappend(pstr, arg, true, 'd');
      servo_sprintf(pstr, format + 2, args...);
      return pstr;
    }
    else if (*format == '%' && *(format + 1) == 's'){
      servo_sappend(pstr, arg, true, 's');
      servo_sprintf(pstr, format + 2, args...);
      format += 2;
    }
    else if (*format == '%' && *(format + 1) == 'z'){
      servo_sappend(pstr, arg);
      servo_sprintf(pstr, format + 2, args...);
      return pstr;
    }
    strncat(pstr, format, 1);
  } while (*(++format) != '\0');
}
