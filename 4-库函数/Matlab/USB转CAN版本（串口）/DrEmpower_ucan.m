classdef DrEmpower_ucan< handle
    properties %成员属性块--开始
        id
        baudrate
        READ_FLAG
    ...
    methods %成员函数块--开始
        % 构造函数（初始化CAN接口）
        function obj = DrEmpower_ucan(id_list,COM)  %%Point2D类的构造函数
            clc
            obj.id = 1;
            obj.baudrate=115200;
            obj.READ_FLAG = 1;
            ...
            obj.dev =serialport(COM,obj.baudrate,"Timeout",0.01);
            flush(obj.dev);
            %             obj.dev.UserData = struct("Data",[]);
            %             configureCallback(obj.dev,"byte",16,@obj.read_data)
            %             %中断回调函数配置
        end
        ...
        function write_data(obj,data)
            ...
            write(obj.dev,data,"uint8");
        end
        
        % 串口接收函数 -中断回调函数
        %         function read_data(obj,src,~)
        %             toc
        %             num=16;
        %             data = read(src,num,"uint8");
        %             obj.dev.UserData.Data = data;
        %             obj.READ_FLAG = 1;
        %         end
        % 串口接收函数 - 阻塞读取
        ...
        function rdata = receive_data(obj)
            obj.READ_FLAG = -1;
            tic;
            while obj.dev.NumBytesAvailable<16 && toc<0.05  % 目前测试大概在2ms以内
            end
        ...
        %设置CAN波特率
        %注意该函数执行后需要使用USB转CAN模块配置软件将CAN波特率同步修改,然后再次调用该函数才能设置成功（永久保存），否则下一次重新上电电机CAN波特率将恢复成原来的值
