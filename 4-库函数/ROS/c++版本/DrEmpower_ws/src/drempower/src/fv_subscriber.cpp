...
void msg_callback(const drempower::fv_msg &msg)
{
    ...
    {
        ROS_INFO("fv_node:[%d,%f,%f]", msg.id_list[i], msg.input_vel_list[i], msg.input_torque_list[i]);
        float_to_data(msg.input_vel_list[i], &data[0]);
        int16_to_data(int16_t(msg.input_torque_list[i] / 0.01f), &data[4]);
        int16_to_data(int16_t(0x01 / 0.01f), &data[6]); // 前馈控制模式mode = 0x01
