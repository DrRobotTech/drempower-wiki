...
void msg_callback(const drempower::tp_msg &msg)
{
    ...
    for (uint8_t i = 0; i < msg.id_list.size(); i++)
    {
        ...
        float input_filter_width = msg.interpolation_freq_list[i] / 2; // CAN控制协议中对应的为角度输入滤波带宽参数，该参数为插补频率的一半；
        int16_to_data(int16_t(0x00 / 0.01f), &data[4]);
        int16_to_data(int16_t(input_filter_width / 0.01f), &data[6]);
