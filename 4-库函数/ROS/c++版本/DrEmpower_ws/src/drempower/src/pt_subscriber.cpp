...
void msg_callback(const drempower::pt_msg &msg)
{
    ...
    {
        ROS_INFO("pt_node:[%d,%f,%f]", msg.id_list[i], msg.input_torque_list[i], msg.torque_ramp_rate_list[i]);
        float_to_data(msg.input_torque_list[i], &data[0]);
        int16_to_data(int16_t(msg.torque_ramp_rate_list[i] / 0.01f), &data[4]);
        int16_to_data(int16_t(0x06 / 0.01f), &data[6]); // 扭矩爬升模式mode = 0x06
