...
# uart = serial.Serial('COM17', 115200)  # 在 windows 下控制电机测试，相应的输入连接的COM口和波特率
#linux系统下
#CH340版本USB转CAN模块（老版）
...
def read_data(num=16):
    ...
    while uart.inWaiting() > 0:
        ...
        byte_list.append(struct.unpack('B', uart.read(1))[0])
...
def get_state(id_num=0):
    ...
    """
    global READ_FLAG, MOTOR_NUM, motor_state
    pos_vel = [0, 0]
    try:
